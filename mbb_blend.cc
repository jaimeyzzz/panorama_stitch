...
MultiBandBlend::MultiBandBlend(const std::vector<cv::Mat>& masks) {
    ...
    // GPU data
    d_dst_mask.upload(dst_mask);
    d_dst_weight_pyr.resize(band_num + 1);
    d_dst_laplace_pyr.resize(band_num + 1);
    for (int layer = 0; layer <= band_num; layer++) {
        d_dst_weight_pyr[layer].upload(dst_weight_pyr[layer]);
        d_dst_laplace_pyr[layer].upload(dst_laplace_pyr[layer]);
    }
    d_src_weight_pyr.resize(stitch_num);
    d_src_laplace_pyr.resize(stitch_num);
    for (int iter = 0; iter < stitch_num; iter++) {
        d_src_weight_pyr[iter].resize(band_num + 1);
        d_src_laplace_pyr[iter].resize(band_num + 1);
        for (int layer = 0; layer <= band_num; layer++) {
            d_src_weight_pyr[iter][layer].upload(src_weight_pyr[iter][layer]);
            d_src_laplace_pyr[iter][layer].upload(src_laplace_pyr[iter][layer]);
        }
    }
}
...
void MultiBandBlend::Blend(const std::vector<GpuMat>& images, GpuMat & result) {
    for (int iter = 0; iter < stitch_num; iter++) {
        GpuMat tmp_image;
        cv::cuda::copyMakeBorder(images[iter], tmp_image, 0, dst_roi.height - images[iter].rows, 0, dst_roi.width - images[iter].cols, cv::BORDER_REFLECT);
        tmp_image.convertTo(d_src_laplace_pyr[iter][0], CV_32FC3);

        for (int layer = 0; layer < band_num; layer++) {
            cv::cuda::pyrDown(d_src_laplace_pyr[iter][layer], d_src_laplace_pyr[iter][layer + 1]);
            cv::cuda::GpuMat up_layer, old_layer;
            cv::cuda::pyrUp(d_src_laplace_pyr[iter][layer + 1], up_layer);
            d_src_laplace_pyr[iter][layer].copyTo(old_layer);
            cv::cuda::subtract(old_layer, up_layer, d_src_laplace_pyr[iter][layer]);
        }
    }
    BlendPyramidLayers();
    for (int layer = band_num; layer > 0; layer--) {
        cv::cuda::GpuMat up_layer, old_layer;
        cv::cuda::pyrUp(d_dst_laplace_pyr[layer], up_layer);
        d_dst_laplace_pyr[layer - 1].copyTo(old_layer);
        cv::cuda::add(old_layer, up_layer, d_dst_laplace_pyr[layer - 1]);
    }
    GpuMat tmp_image;
    d_dst_laplace_pyr[0](cv::Rect(0, 0, dst_mask.cols, dst_mask.rows)).copyTo(tmp_image, d_dst_mask);
    tmp_image.convertTo(result, CV_8UC3);
}
