...
std::vector<cv::Mat> PanoRemap::get_masks() {
    return masks;
}
...
#define	A	(-0.75)
// 0 <= x < 1
inline double PanoRemap::Cubic01(double x) {
    return	((A + 2.0)*x - (A + 3.0))*x*x + 1.0;
}
// 1 <= x < 2
inline double PanoRemap::Cubic12(double x) {
...
void PanoRemap::initial_remap_table() {
...
void PanoRemap::Transform(int idx, double x, double y, double* dx, double* dy) {
